...
# function for identifying each grid's corresponding county 
...
# this is just for demo; here we only consider data within the boundary
lat_s = 21.9000
lat_n = 25.3000
lng_w = 120.0500
lng_e = 121.9500
...
# front-end design
st.title('預定點業績預測與推薦系統（演示版）')

st.sidebar.header('請輸入預定點基本資料')
with st.sidebar.form(key ='form1'):
    set_lat = st.text_input(label = '預定點緯度', placeholder = '範例格式: 23.9588')
    set_lng = st.text_input(label = '預定點經度', placeholder = '範例格式: 120.5528')
    sells_ground = st.text_input(label = '預估商場坪數', placeholder = '範例格式: 45')
    storehous_ground = st.text_input(label = '預估倉庫坪數', placeholder = '範例格式: 10')
    op_time = st.selectbox('預估營業時間類型', ('24小時', '非24小時'))   
    submitted1 = st.form_submit_button(label = '送出資料進行搜尋')

if submitted1:
    with st.spinner('該預定點業績預估中，同時系統將搜索其他地點進行推薦......'):  
        if float(set_lng) < lng_w or float(set_lng) > lng_e or float(set_lat) < lat_s or float(set_lat) > lat_n:
            st.error('請填入位於台灣範圍內的經緯度位址：經度介於120.0500至121.9500；緯度介於21.9000至25.3000')
